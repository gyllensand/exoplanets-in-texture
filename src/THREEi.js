...
(function (global, factory) {
  ...
  function createSphereWithHoles(arg1, arg2) {
    ...
    if (arg1.d > 0) {
      // variant parameters object { d: div4: holes: } all elements optional
      ...
      // variant detail, holes are optional,
  ...
  function buildSphereWithHolesObj() {
    ...
    const determinant = (xa, ya, za, xb, yb, zb, xc, yc, zc) =>
      xa * yb * zc +
      ya * zb * xc +
      za * xb * yc -
      za * yb * xc -
    ...
    function checkDistancesToUnite(m) {
      // for new active front points
    ...
    function checkDistancesToSplit(m) {
      // for new active front points
      ...
      for (let i = 0; i < front.length; i++) {
        for (let j = 0; j < n; j++) {
          // check n new points (insertFront)
    ...
    function coordTangentialSystem() {
      ...
      xs1 =
        determinant(x1 - xp, y1 - yp, z1 - zp, xt2, yt2, zt2, xn, yn, zn) / det;
      ys1 =
        determinant(xt1, yt1, zt1, x1 - xp, y1 - yp, z1 - zp, xn, yn, zn) / det;
      //zs1 = determinant( xt1, yt1, zt1, xt2, yt2, zt2, x1 - xp, y1 - yp, z1 - zp ) / det; // not needed

      xs2 =
        determinant(x2 - xp, y2 - yp, z2 - zp, xt2, yt2, zt2, xn, yn, zn) / det;
      ys2 =
        determinant(xt1, yt1, zt1, x2 - xp, y2 - yp, z2 - zp, xn, yn, zn) / det;
  ...
  function buildCylinderWithHoles() {
    ...
    const prevFront = (i) => (i !== 0 ? i - 1 : front.length - 1);
    const nextFront = (i) => (i !== front.length - 1 ? i + 1 : 0);
    const detYc0 = (xa, ya, za, xb, yb, zb, xc, zc) =>
      xa * yb * zc + ya * zb * xc - za * yb * xc - ya * xb * zc; // determinant yc = 0;
        ...
        y =
          ...
          sign *
            (-rAdp +
              Math.sqrt(
                rAdp * rAdp -
                  g.radius * g.radius * Math.cos(phi) * Math.cos(phi)
              ));
    ...
    function makeCircularHole(i) {
      ...
      for (
        ...
        x0 = Math.sqrt(
          g.radius * g.radius -
            rHole * rHole * Math.cos(alpha) * Math.cos(alpha)
        );
  ...
  function createInnerGeometry(p) {
    ...
    function parametersToSurface() {
      ...
      if (g.surface === "outline") {
        g.d = p.d !== undefined ? p.d : 0.1;
        g.points =
          p.points !== undefined ? p.points : [1, 1, -1, 1, -1, -1, 1, -1];

        g.detail = 8 / g.d;
      }

      if (g.surface === "planecap") {
        // ellipse (circle) as bottom or top of a tilted (0) cut cylinder
      ...
      if (g.surface === "cylinder") {
        ...
        g.div4Btm =
          p.div4Btm !== undefined ? p.div4Btm : Number.MAX_SAFE_INTEGER;
        ...
        g.div4Top =
          p.div4Top !== undefined ? p.div4Top : Number.MAX_SAFE_INTEGER;
  ...
  function buildInnerGeometry(p) {
      ...
      if (
        ...
        for (let i = 0; i < g.holes.length; i++) {
          if (g.holes[i][0] === "circle") {
            ...
            for (
              let j = 0, psi = Math.PI * 2;
              j < g.holes[i][1] * 4;
              j++, psi -= Math.PI / g.holes[i][1] / 2
            ) {
            ...
            for (
              let j = 0, psi = Math.PI * 2;
              j < g.holes[i][1];
              j++, psi -= (Math.PI * 2) / g.holes[i][1]
            ) {
              outline.push(
                r0 * Math.cos(psi) + g.holes[i][3],
                r0 * Math.sin(psi) + g.holes[i][4]
              );
            }
    ...
    function makeFrontPos(xyzCalculation) {
      ...
      while (
        sign > 0
          ? psi < psiEnd &&
            (sqlen1 > 5.76 * dd || psi < (psiStart + psiEnd) / 2)
          : psi > psiEnd &&
            (sqlen1 > 5.76 * dd || psi > (psiStart + psiEnd) / 2)
      ) {
        psi0 = psi;
    ...
    function cylinder_makeCylinderHole(i) {
      ...
      if (g.radius + g.rAdp - exc > 0) {
        ...
        if (r0 + exc > r1) {
          // a connected hole, side is ignored
        ...
        if (
          ...
          // two separate holes (second: side === '-')
    ...
    function cylinder_xyzCylinderBoundOrHole() {
      // uses r0, r1
      ...
      for (let j = 1; j < jMax; j++) {
        ...
        switch (g.surface) {
          ...
          case "outline":
            x2 =
              i < 0
                ? outline[j < jMax - 1 ? 2 * j : 0]
                : g.holes[i][j < jMax - 1 ? 2 * j : 0]; // 0 => connect to start
            y2 = 0;
            z2 =
              i < 0
                ? outline[j < jMax - 1 ? 2 * j + 1 : 1]
                : g.holes[i][j < jMax - 1 ? 2 * j + 1 : 1]; // 1 => connect to start;
  ...
  function buildImplicitSurface() {
    ...
    function triangulation(isf, dx, dy, dz, xs, ys, zs, d, e, fc, pc) {
      ...
      function coordTangentialSystem() {
        ...
        xs1 =
          determinant(x1 - xp, y1 - yp, z1 - zp, xt2, yt2, zt2, xn, yn, zn) /
          det;
        ys1 =
          determinant(xt1, yt1, zt1, x1 - xp, y1 - yp, z1 - zp, xn, yn, zn) /
          det;
        //zs1 = determinant( xt1, yt1, zt1, xt2, yt2, zt2, x1 - xp, y1 - yp, z1 - zp ) / det; // not needed

        xs2 =
          determinant(x2 - xp, y2 - yp, z2 - zp, xt2, yt2, zt2, xn, yn, zn) /
          det;
        ys2 =
          determinant(xt1, yt1, zt1, x2 - xp, y2 - yp, z2 - zp, xn, yn, zn) /
          det;
        //zs2 = determinant( xt1, yt1, zt1, xt2, yt2, zt2, x2 - xp, y2 - yp, z2 - zp ) / det; // not needed
      }
    ...
    function triangulationBounds(isf, dx, dy, dz, xs, ys, zs, d, e, fc, pc, b) {
      ...
      function getMinimalAngleIndex() {
        ...
        for (let i = 0; i < front.length; i++) {
          if (front[i].ang < angle && !front[i].bou) {
            // not for boundary point
